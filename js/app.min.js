    ...
    if (ranges.length) {
        ranges.forEach((range) => {
            ...
            input.addEventListener('input', () => {
                ...
                percent = ((input.value - input.min) / (input.max - input.min) * 100);
                thumb.style.left = percent + '%';
                value.innerHTML = numberFormat(input.value);
                rangesCombine(input);
                calc();
            });
        });
...
const rangesCombine = (input) => {
    const listItem = input.closest('.form-calc__item');
    let data = '';
    let value = '';

    if (listItem.dataset.calc == 'power') {
        data = 'summ';
        value = input.value * 0.5 * 1000;
    } else {
        data = 'power';
        value = input.value / 1000 / 0.5;
    }

    const range = listItem.closest('.calc').querySelector(`[data-calc=${data}] `);    

    const rangeInput = range.querySelector('input');
    const rangeValue = range.closest('.form-calc__item').querySelector('span');
    const rangeThumb = range.querySelector('.range-single__thumb');
    let percent = '';

    rangeInput.value = value;
    rangeValue.innerHTML = numberFormat(value);
...
const calc = () => {
    ...
    if (calcs.length) {
        calcs.forEach((calc) => {
            let summInput = calc.querySelector('[data-calc=summ] input').value;
            let powerInput = calc.querySelector('[data-calc=power] input').value;
            let index = calc.querySelector('[data-calc=index] input').value;

            let volumeYear = calc.querySelector('[data-calc=year-volume] span');
            let volumeMonth = calc.querySelector('[data-calc=month-volume] span');
            let term = calc.querySelector('[data-calc=term] span');
            let savingYear = calc.querySelector('[data-calc=year-saving] span');
            let savingMonth = calc.querySelector('[data-calc=month-saving] span');
            let square = calc.querySelector('[data-calc=square] span');

            let volumeYearValue = (powerInput * 1.08 * 1000).toFixed(2);
            let volumeMonthValue = (volumeYearValue / 12).toFixed(2);
            let savingYearValue = (volumeYearValue * index).toFixed(2);
            let savingMonthValue = (savingYearValue / 12).toFixed(2);
            let squareValue = (powerInput * 6.8).toFixed(2);
            let termValue = (summInput / savingYearValue).toFixed(2);

            volumeYear.innerHTML = numberFormat(volumeYearValue);
            volumeMonth.innerHTML = numberFormat(volumeMonthValue);
            savingYear.innerHTML = numberFormat(savingYearValue);
            savingMonth.innerHTML = numberFormat(savingMonthValue);
            square.innerHTML = numberFormat(squareValue);
...
const sidebarFix = () => {
    ...
    if (sidebar && window.innerWidth > 768) {
        ...
        const sidebarShare = document.querySelector('.sidebar__share');

        if (scroll > top - 24) {
            sidebarShare.classList.add('sticky');
        } 
        if (scroll < top - 24) {
            sidebarShare.classList.remove('sticky');
        } 
        if (scroll > bottom - sidebarShare.scrollHeight - 24) {
            sidebarShare.classList.add('fixed');
        } else {
            sidebarShare.classList.remove('fixed');
        }        
    } 
}
...
document.addEventListener('click', (e) => {
    ...
    const copyBtn = e.target.closest('.sidebar-copy');
    if (copyBtn) {
        copyLink(e);
    }
});
...
document.addEventListener('wpcf7mailsent', (e) => {
    ...
    const modalDone = document.querySelector('#thanks');
    if (modalActive) {
